...
type ProjectSpecInsert = Database['public']['Tables']['project_specs']['Insert']
...
export async function generateProjectDraft(formData: {
  ...
  const specData: ProjectSpecInsert = {
    user_id: user.id,
    budget_global: formData.budget_global,
    ...
    // Mapping basique des restrictions pour l'exemple
    venue_floor_without_elevator: formData.restrictions.includes('pmr'), 
    dietary_restrictions: formData.restrictions.includes('alimentaire') ? ['halal', 'vegan'] : [],
  ...
  if (specError) {
    console.error("Erreur insertion project_specs:", specError)
    return { error: "Erreur lors de la sauvegarde de votre projet." }
  }
  ...
  for (const category of requiredCategories) {
    const { data: services } = await supabase
      .from('services')
      .select('id, title, description, category, city, price_fixed, price_per_day, price_per_hour, images, profiles(first_name, last_name)')
